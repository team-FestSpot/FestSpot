...
#### 관리자 페이지 접속 하면 리스트 api 받아와서 화면 그리고 공연을 유저 페이지로 올릴때 상세 정보 api 같이 받아와서 두 데이터 합친 후 db에 넣는것 까지는 똑같음 그런데 유저 페이지에 올라가 있는 공연만 아침 8시부터 저녁 10시까지만 1시간 간격으로 데이터 가져와서 업데이트 확인함 이렇게 하면 유저가 클릭 할때마다 요청 안날려도 됨 대신 공연이 50개만 되도 하루에 750건 요청 보냄 (키당 하루 제한 1500건) (연말 대비 키 2~3개 추가 필요 할지도)