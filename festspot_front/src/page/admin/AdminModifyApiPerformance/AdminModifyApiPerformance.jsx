...
import AdminApiPerformanceDataGrid from "../../../components/admin/AdminDataGrid/AdminApiPerformanceDataGrid/AdminApiPerformanceDataGrid";
import { useApiPerformanceListQuery } from "../../../querys/admin/useApiPerformanceListQuery";

function AdminModifyApiPerformance(props) {
  const { data, refetch } = useApiPerformanceListQuery();
  ...
  const handleSearchResetButtonOnClick = async () => {
    ...
    await queryClient.invalidateQueries(["getApiPerformanceList"]);
    setRowsEmpty();
  ...
  return (
    <div css={s.layout}>
      ...
      <main>
        <div>
          <AdminApiPerformanceDataGrid searchResult={searchResult} />
        </div>
      </main>
    </div>
  );
...
export default AdminModifyApiPerformance;
