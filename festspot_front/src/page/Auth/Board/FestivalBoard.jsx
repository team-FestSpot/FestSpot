...
function FestivalBoard() {
  ...
  const currentPagePosts = currentBoardData.posts || [];
  const totalPages = currentBoardData.totalPages || 1;
  ...
  return (
    ...
    //           key={post.postId}
    //           css={s.card}
    //           onClick={() => handlePageOnClick(post.postId)}
    //         >
    ...
    //   )}
    //   {/* 빈 상태 */}
    //   {boardPosts.isFetched && currentPagePosts.length === 0 && (
    ...
    //   {/* 페이지네이션 - 항상 표시 */}
    //   {!boardPosts.isLoading && !boardPosts.isError && (
